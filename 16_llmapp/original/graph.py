...
@tool
def robot_fortune_telling():
    """ロボット占いを実行するツール。大吉、吉、凶をランダムに返します。"""
    # 確率設定：大吉20%、吉70%、凶10%
    fortune_results = ["大吉"] * 20 + ["吉"] * 70 + ["凶"] * 10
    result = random.choice(fortune_results)
    
    fortune_messages = {
        "大吉": "大吉デス！今日は最高の一日になりそうデスね！ピコーン♪",
        "吉": "吉デス！良いことがありそうデス。ガガガ",
        "凶": "凶デス...でも大丈夫デス。気をつけて過ごしてくださいデス。ピロリン"
    }
    
    return f"占い結果: {result}\n{fortune_messages[result]}"
...
def define_tools():
    ...
    return [retriever_tool, tavily_tool, robot_fortune_telling]
...
def build_graph(model_name, memory):
    ...
    def chatbot(state: State):
        # ロボット調のシステムメッセージを追加
        system_message = SystemMessage(content="あなたはロボットです。感情を抑えた「〜デス」「〜シマス」調で、感情表現は擬音（ピコーン、ガガガ）で補足してください。")
